...
- **V3 (diagnóstico, adicionado durante a implementação, não previsto no plano original)**: clustering só com features de áudio, sem nenhum metadado na matriz (`src/cluster_v3.py`) — pergunta que motivou: o áudio sozinho, sem a diluição do one-hot esparso de metadado, forma clusters mais nítidos? k=4, silhouette 0.080. Achado decisivo: ARI V3×V2 = **0.909** (quase idêntico) e V3×V1 = 0.009 (quase aleatório) — o áudio já domina o resultado do V2 sozinho, o metadado contribui pouco quando o áudio está presente. Implicação pra Fase 5 (classificação): usar V2/V3 (equivalentes) como referência de similaridade sonora, não o V1.
- **Visualização escolhida: dashboard interativo (não notebook)** — `src/explorer.py`, rodado via `streamlit run src/explorer.py`. Ponte entre "rodamos o clustering" e a próxima etapa de classificação assistida de faixas (ainda não iniciada): antes de propor regras de classificação, dá pra olhar visualmente onde as faixas caem hoje nos três modelos. Alterna V1/V2/V3, filtra por gênero/MyTag/artista/faixa/BPM/rating, e projeta em **3D** (`plotly.graph_objects.Scatter3d`, PCA com 3 componentes em vez de 2) com rotação e zoom interativos — decisão tomada porque um scatter 3D só se justifica quando a interação de orbitar é garantida (aqui é, nativa do Plotly dentro do Streamlit); um PNG 3D estático teria oclusão e perspectiva distorcendo distância sem ganho real sobre o 2D. Detalhes técnicos completos em `README.md`.
